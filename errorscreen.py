...
class ErrorScreen:
    ...
    def draw(self, screen: pygame.Surface):
        if not self.active:
            return
        error_msg_surface = get_font("./assets/poppins-regular.ttf", 18).render(self.msg, True, get_colors()["hallway"])  # todo: cache the error text
        press_esc_to_exit = get_font("./assets/poppins-regular.ttf", 24).render("Press escape to go back", True, get_colors()["hallway"])
